...
public class UserMealsUtil {
    ...
    public static Map<LocalDate, Integer> calPerDay(List<UserMeal> meals) {
        ...
        meals.stream().forEach(x -> mapCalPerDay.put(x.getDateTime().toLocalDate(), mapCalPerDay.getOrDefault(x.getDateTime().toLocalDate()
                , 0) + x.getCalories()));
        return mapCalPerDay;
    }
    ...
    public static List<UserMealWithExcess> filteredByStreams3(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
              return  meals.stream().collect(Collectors.groupingBy(UserMeal::getDate))
                .values()
                .stream().collect(
                        Collectors.toMap(
